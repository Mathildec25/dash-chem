...
# =============================================================================
# HELPER — LOAD AND PREPARE SOURCE EXPERIMENTS FOR TRANSFER LEARNING
# =============================================================================

def _load_source_experiments(source_excel, param_names, obj_names, param_definitions):
    """
    Load and prepare experiments from a source campaign for transfer learning.

    Args:
        source_excel      : str  — Excel filename of the source campaign
        param_names       : list — parameter column names (must match current domain)
        obj_names         : list — objective column names (must match current domain)
        param_definitions : dict — {param_name: param_def} from current domain

    Returns:
        pd.DataFrame or None — complete source experiments, or None on failure
    """
    ...
    # Apply same type conversions as the target campaign
    ...
    # Keep only rows where all objectives are present
...
def toggle_modals(add_open, add_cancel, add_confirm, del_open, del_cancel, del_confirm,
                  add_is_open, del_is_open):
...
def add_new_column(n_clicks, col_name, default_value, columns, data):
    ...
    new_col = {'name': col_name, 'id': col_name, 'editable': True, 'type': 'text'}
...
def run_bayesian_optimization(n_clicks, table_data, excel_file, advanced_settings):
    ...
    try:
        ...
        # Reconstruct discretization config
            ...
            if col in param_names:
                ...
                if param_def:
                    ...
                    if param_type == 'cat':
                        ...
                        print(f"🏷️ Categorical '{col}' — allowed: {allowed_values}")
        ...
        print(f"📈 Complete experiments: {len(experiments_complete)} rows")
        ...
        if constraints_config and constraints_config.get('constraints') and solvent_param_name:
            ...
            for idx, row in new_candidates.iterrows():
                ...
                adjusted_row, adjustments = validate_and_adjust_suggestion(
                    row_dict, constraints_config, solvent_param_name
                )
        ...
        for _, candidate in new_candidates.iterrows():
            ...
            for col in df.columns:
                if col == 'Point type':
                    new_row[col] = 'BO (TL)' if source_experiments is not None else 'BO'
        ...
        tl_note = f" (Transfer Learning from '{tl_config.get('source_campaign')}')" \
                  if source_experiments is not None else ""
        if all_adjustments:
            adjustment_text = ", ".join([
                f"{adj['parameter']} → {adj['adjusted']}°C (max for {adj['solvent']})"
                for adj in all_adjustments
            ])
            msg = f"✅ Generated {n_suggestions} new experiment(s){tl_note}! ⚠️ Adjusted: {adjustment_text}"
        else:
            msg = f"✅ Generated {n_suggestions} new experiment(s){tl_note} to test!"
